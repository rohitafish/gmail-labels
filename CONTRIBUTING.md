...
pip-compile** (pip-tools): every direct *and transitive* package -- there
are about thirty behind the two Google client libraries -- at an exact
...
`pip-audit` against the lock on every push and on a weekly schedule, so a new
advisory against a pinned version shows up without anyone remembering to
check. Running the scripts themselves against
