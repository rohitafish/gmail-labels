...
silently drops platform-only packages and markers (`colorama` on Windows,
for one) even on a file whose header still reads "autogenerated by uv ...
--universal". Harmless here (this repo only ever runs on Unix), but it's
exactly the kind of narrowing that broke CI once already (see the sibling
`home-asset-manager` repo's git history for the SQLAlchemy `greenlet`
incident). So: after merging a batch of Dependabot bumps, run the
regenerate command above by hand once to restore full platform coverage,
rather than trusting Dependabot's own commit as the final word. If a
Dependabot PR ever comes through with a bare pin and no hashes at all,
regenerate by hand before merging that too. Installing with
`--require-hashes` (what CI does) means a package whose published file
doesn't match the recorded hash is refused rather than run. CI also runs
`pip-audit` against the lock on every push and on a weekly schedule, so a
new advisory against a pinned version shows up without anyone remembering
to check. Running the scripts themselves against
