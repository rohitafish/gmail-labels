...
a `uv`-generated lock and regenerates it in its own PRs; if a Dependabot PR
ever comes through with a bare pin and no hashes, regenerate by hand with
the command above before merging. Installing with `--require-hashes` (what
CI does) means a package whose published file doesn't match the recorded
hash is refused rather than run. CI also runs `pip-audit` against the lock
on every push and on a weekly schedule, so a new advisory against a pinned
version shows up without anyone remembering to check. Running the scripts themselves against
